...
DFM2_INLINE void delfem2::EMat_SolidDynamicLinear_Tri2D(
  ...
  if (is_initial) {
    for (int ino = 0; ino < nno; ino++) {
      ...
      for (int jno = 0; jno < nno; jno++) {
        eres[ino][0] -=
          0.5 * dt_timestep * dt_timestep * (eKmat[ino][jno][0][0] * acc[jno][0] + eKmat[ino][jno][0][1] * acc[jno][1]);
        eres[ino][1] -=
          0.5 * dt_timestep * dt_timestep * (eKmat[ino][jno][1][0] * acc[jno][0] + eKmat[ino][jno][1][1] * acc[jno][1]);
      }
    }
  }
}
...
DFM2_INLINE void delfem2::elemMatRes_LinearSolidGravity3_Static_Q1(
  ...
  constexpr int nw0 = kNumIntegrationPoint_GaussianQuadrature[iGauss];
  constexpr int nw1 = kNumIntegrationPoint_GaussianQuadrature[iGauss + 1];

  std::fill_n(&emat[0][0][0][0], 8 * 8 * 3 * 3, 0.0);
  std::fill_n(&eres[0][0], 8 * 3, 0.0);

  double vol = 0.0;
  for (int ir1 = nw0; ir1 < nw1; ir1++) {
    for (int ir2 = nw0; ir2 < nw1; ir2++) {
      for (int ir3 = nw0; ir3 < nw1; ir3++) {
        const double r1 = kPositionWeight_GaussianQuadrature<double>[ir1][0];
        const double r2 = kPositionWeight_GaussianQuadrature<double>[ir2][0];
        const double r3 = kPositionWeight_GaussianQuadrature<double>[ir3][0];
        double dndx[8][3], an[8], detjac;
        ShapeFunc_Hex8(r1, r2, r3, coords, detjac, dndx, an);
        const double w1 = kPositionWeight_GaussianQuadrature<double>[ir1][1];
        const double w2 = kPositionWeight_GaussianQuadrature<double>[ir2][1];
        const double w3 = kPositionWeight_GaussianQuadrature<double>[ir3][1];
        const double detwei = detjac * w1 * w2 * w3;
        vol += detwei;
