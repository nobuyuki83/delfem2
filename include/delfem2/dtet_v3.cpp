...
std::vector<int>& tmp_buffer)
{
  ...
  std::vector<COld> aOld;
  {
    ...
    const dfm2::CVec3& p_ins = aPo3D[ip_ins].p;
...
//    std::cout<<"ntet_cur:"<<aSTet.size()<<"  ntet_del:"<<ntet_old<<"  ntet_new:"<<ntet_new<<std::endl;
  ...
  for (unsigned int inew = 0; inew<aNew.size(); ++inew){
    ...
    { // assert volume is positive
      ...
      const dfm2::CVec3& p0 = aPo3D[i0].p;
      const dfm2::CVec3& p1 = aPo3D[i1].p;
      const dfm2::CVec3& p2 = aPo3D[i2].p;
      const dfm2::CVec3& p3 = aPo3D[i3].p;
