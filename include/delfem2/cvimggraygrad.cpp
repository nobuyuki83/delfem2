  ...
  for (int iv = 0; iv < nv; ++iv) {
    for (int iy1 = 0; iy1 < ny; ++iy1) {
      for (int ix1 = 0; ix1 < nx; ++ix1) {
        aGray(iv,iy1,ix1) = Dot3(frame.data(iv, iy1, ix1), gray_coeff);
      }
    }
  }
}
  ...
  for(int iv = 0; iv < nv; ++iv){
    for (int iy1 = 0; iy1 < ny; ++iy1) {
      for (int ix1 = 0; ix1 < nx; ++ix1) {
        ...
        assert( ix1 >=0 && ix1 < nx );
        assert( iy1 >=0 && iy1 < ny );
        const auto v01 = Dot3(frame.data(iv,iy0,ix1),gray_coeff);
        const auto v10 = Dot3(frame.data(iv,iy1,ix0),gray_coeff);
        const auto v12 = Dot3(frame.data(iv,iy1,ix2),gray_coeff);
        const auto v21 = Dot3(frame.data(iv,iy2,ix1),gray_coeff);
  ...
  for(int iv = 0; iv < nv; ++iv){
    for (int iy1 = 0; iy1 < ny; ++iy1) {
      for (int ix1 = 0; ix1 < nx; ++ix1) {
        ...
        assert( ix1 >=0 && ix1 < nx );
        assert( iy1 >=0 && iy1 < ny );
        const auto v00 = Dot3(frame.data(iv,iy0,ix0),gray_coeff);
        const auto v01 = Dot3(frame.data(iv,iy0,ix1),gray_coeff);
        const auto v02 = Dot3(frame.data(iv,iy0,ix2),gray_coeff);
        const auto v10 = Dot3(frame.data(iv,iy1,ix0),gray_coeff);
        const auto v12 = Dot3(frame.data(iv,iy1,ix2),gray_coeff);
        const auto v20 = Dot3(frame.data(iv,iy2,ix0),gray_coeff);
        const auto v21 = Dot3(frame.data(iv,iy2,ix1),gray_coeff);
        const auto v22 = Dot3(frame.data(iv,iy2,ix2),gray_coeff);
