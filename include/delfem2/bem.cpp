    ...
    const std::vector<double>& aXYZ)
{
  ...
  dfm2::CVec3 v0(aXYZ[i0*3+0], aXYZ[i0*3+1], aXYZ[i0*3+2]);
  dfm2::CVec3 v1(aXYZ[i1*3+0], aXYZ[i1*3+1], aXYZ[i1*3+2]);
  dfm2::CVec3 v2(aXYZ[i2*3+0], aXYZ[i2*3+1], aXYZ[i2*3+2]);
  dfm2::CVec3 n; Normal(n, v0, v1, v2);
  return n;
}
...
const std::vector<double>& aSolidAngle)
{
  ...
  for (int ip = 0; ip<np; ip++){
    dfm2::CVec3 p(aXYZ[ip*3+0], aXYZ[ip*3+1], aXYZ[ip*3+2]);
    for (int jtri = 0; jtri<(int)aTri.size()/3; ++jtri){
      ...
      const dfm2::CVec3 q0(aXYZ[jq0*3+0], aXYZ[jq0*3+1], aXYZ[jq0*3+2]);
      const dfm2::CVec3 q1(aXYZ[jq1*3+0], aXYZ[jq1*3+1], aXYZ[jq1*3+2]);
      const dfm2::CVec3 q2(aXYZ[jq2*3+0], aXYZ[jq2*3+1], aXYZ[jq2*3+2]);
      dfm2::CVec3 n = Normal(q0, q1, q2);
      ...
      for (int iint = 0; iint<nint; iint++){
        ...
        dfm2::CVec3 yb = r0*q0+r1*q1+r2*q2;
        dfm2::CVec3 v = (p-yb);
        double len = v.Length();
...
dfm2::CVec3 evaluateField_PotentialFlow_Order1st
(double& phi_pos,
const dfm2::CVec3& pos,
const dfm2::CVec3& velo_inf,
int ngauss,
...
{
  const int np = (int)aXYZ.size()/3;
  if (aValSrf.size()!=np){ return dfm2::CVec3(0, 0, 0); }
  dfm2::CVec3 gradphi_pos = dfm2::CVec3(0, 0, 0);
  phi_pos = 0;
  for (int jtri = 0; jtri<(int)aTri.size()/3; ++jtri){
    const int jq0 = aTri[jtri*3+0];
    const int jq1 = aTri[jtri*3+1];
    ...
    for (int iint = 0; iint<nint; iint++){
      ...
      double G = 1.0/(4*M_PI*len);
      double dGdn = (v*n)/(4*M_PI*len*len*len);
      dfm2::CVec3 dGdx = -v/(4*M_PI*len*len*len);
      dfm2::CVec3 dGdndx = (1/(4*M_PI*len*len*len))*n-(3*(v*n)/(4*M_PI*len*len*len*len*len))*v;
      ...
      {
        dfm2::CVec3 gradphyx = dGdndx*phiyb-dGdx*vnyb;
    ...
    const std::vector<unsigned int> &aTri)
{
  ...
  for (std::size_t it = 0; it<nt; it++){
    const dfm2::CVec3 pm = MidPoint(it, aTri, aXYZ);
    ...
    const std::vector<unsigned int> &aTri)
{
  ...
  for (int jtri = 0; jtri<nt; ++jtri){
    ...
    for (int iint = 0; iint<nint; iint++){
      ...
      const dfm2::CVec3 yb = r0*q0+r1*q1+r2*q2;
      const dfm2::CVec3 r = (pos-yb);
      const double len = r.Length();
      double G = 1.0/(4*M_PI*len);
      double dGdny = (r*ny)/(4*M_PI*len*len*len);
      dfm2::CVec3 dGdx = -r/(4*M_PI*len*len*len);
      dfm2::CVec3 dGdnydx = (1/(4*M_PI*len*len*len))*ny-(3*(r*ny)/(4*M_PI*len*len*len*len*len))*r;
      ///
...
//      dGdnydx *= reg;
      ...
      {
        dfm2::CVec3 gradphyx = -dGdnydx*phiy+dGdx*vnyb;
...
(double aC[4], 
const dfm2::CVec3& x0, const dfm2::CVec3& x1, const dfm2::CVec3& x2,
const dfm2::CVec3& y0, const dfm2::CVec3& y1, const dfm2::CVec3& y2,
const dfm2::CVec3& velo,
int ngauss)
{
  assert(ngauss>=0&&ngauss<6);
  const int nint = NIntTriGauss[ngauss]; // number of integral points
  //
  dfm2::CVec3 xm = (x0+x1+x2)*0.3333333333333333333;
  dfm2::CVec3 nx = Normal(x0, x1, x2);
//  const double areax = nx.Length()*0.5; // area
  nx.SetNormalizedVector(); // unit normal
  const dfm2::CVec3 ux = (x1-x0).Normalize();
  const dfm2::CVec3 vx = nx^ux;
  //
  dfm2::CVec3 ny = Normal(y0, y1, y2);
  const double areay = ny.Length()*0.5; // area
  ny.SetNormalizedVector(); // unit normal
  const dfm2::CVec3 uy = (y1-y0).Normalize();
  const dfm2::CVec3 vy = ny^uy;
  //
  ...
  for (int iint = 0; iint<nint; iint++){
    ...
    double r2 = 1.0-r0-r1;
    double wb = TriGauss[ngauss][iint][2];
    dfm2::CVec3 yb = r0*y0+r1*y1+r2*y2;
    dfm2::CVec3 r = (xm-yb);
    double len = r.Length();
//    double G = -1.0/(4*M_PI*len);
//    double dGdn = -(r*ny)/(4*M_PI*len*len*len);
    dfm2::CVec3 dGdy = -r/(4*M_PI*len*len*len);
    dfm2::CVec3 pvycdGdy = -(vy^r)/(4*M_PI*len*len*len); // +vy ^ dGdy = (ny^uy)^dGdy
    dfm2::CVec3 muycdGdy = +(uy^r)/(4*M_PI*len*len*len); // -uy ^ dGdy = (ny^vy)^dGdy
    {
...
const std::vector<int>& aTri)
{
  ...
  for (int it = 0; it<nt; ++it){
    ...
    const dfm2::CVec3 p0(aXYZ[ip0*3+0], aXYZ[ip0*3+1], aXYZ[ip0*3+2]);
    const dfm2::CVec3 p1(aXYZ[ip1*3+0], aXYZ[ip1*3+1], aXYZ[ip1*3+2]);
    const dfm2::CVec3 p2(aXYZ[ip2*3+0], aXYZ[ip2*3+1], aXYZ[ip2*3+2]);
    {
      const dfm2::CVec3 nx = Normal(p0, p1, p2).Normalize();
      const dfm2::CVec3 ux = (p1-p0).Normalize();
      const dfm2::CVec3 vx = (nx^ux);
      f[it*2+0] = ux*velo;
      f[it*2+1] = vx*velo;
    }
...
dfm2::CVec3 evaluateField_VortexSheet_Order0th
(const dfm2::CVec3& pos,
 const std::vector<double>& aValSrf,
 //
 int ngauss,
 ...
 int jtri_exclude)
{
  ...
  for (int jt = 0; jt<nt; ++jt){
    ...
    const double areay = ny.Length()*0.5; // area
    ny.SetNormalizedVector(); // unit normal
    const dfm2::CVec3 uy = (q1-q0).Normalize();
    const dfm2::CVec3 vy = ny^uy;
    const int nint = NIntTriGauss[ngauss]; // number of integral points
    for (int iint = 0; iint<nint; iint++){
      const double r0 = TriGauss[ngauss][iint][0];
...
dfm2::CVec3 veloVortexParticle
(const dfm2::CVec3& pos_eval,
const dfm2::CVec3& pos_vp,
const dfm2::CVec3& circ_vp,
double rad_vp)
{
  dfm2::CVec3 v = pos_eval-pos_vp;
  double len = v.Length();
  double ratio = len/rad_vp;
  double f0 = 1.0-exp(-ratio*ratio*ratio);
//  double f0 = 1.0;
  double g0 = f0/(4*M_PI*len*len*len);
  dfm2::CVec3 k0 = g0*(circ_vp^v);
  return g0*(circ_vp^v);
}

dfm2::CMat3d gradveloVortexParticle
(dfm2::CVec3& velo_eval,
const dfm2::CVec3& pos_eval,
const dfm2::CVec3& pos_vp,
const dfm2::CVec3& circ_vp,
double rad_vp)
{
  ...
  ///
  dfm2::CVec3 dlen = v.Normalize();
  dfm2::CVec3 dratio = dlen/rad_vp;
  dfm2::CVec3 df0 = (exp(-ratio*ratio*ratio)*3*ratio*ratio)*dratio;
  dfm2::CVec3 dg0 = (1.0/(4*M_PI*len*len*len))*df0-(3*f0/(4*M_PI*len*len*len*len))*dlen;
  return dfm2::Mat3_OuterProduct(circ_vp^v, dg0)+g0*dfm2::Mat3_Spin(circ_vp);
}

dfm2::CVec3 veloVortexParticles
(const dfm2::CVec3& p0,
...
(std::vector<CVortexParticle>& aVortexParticle, CGrid_Vortex& grid, double resolution)
{
  ...
  for (int ivp = 1; ivp<aVortexParticle.size(); ++ivp){
    dfm2::CVec3 p = aVortexParticle[ivp].pos;
  ...
  grid.cnt = dfm2::CVec3(min_x+max_x, min_y+max_y, min_z+max_z)*0.5;
  max_x += h; 
  max_y += h;
  max_z += h;
  min_x -= h;
    ...
    bool is_inverted_norm)
{
  ...
  for (int jtri = 0; jtri<ntri; jtri++){
    dfm2::CVec3 pmj = MidPoint(jtri, aTri, aXYZ);
    double rm = (p-pmj).Length();
    dfm2::CVec3 n = NormalTri(jtri, aTri, aXYZ);
...
(COMPLEX aC[3],
const dfm2::CVec3& p0,
const dfm2::CVec3& q0, const dfm2::CVec3& q1, const dfm2::CVec3& q2,
...
{  
  ...
  for (int iint = 0; iint<nint; iint++){
    ...
    dfm2::CVec3 v = p0-(r0*q0+r1*q1+r2*q2);
    double d = v.Length();  
...
int ngauss)
{
  ...
  for (int jtri = 0; jtri<ntri; jtri++){
    ...
    dfm2::CVec3 q0(aXYZ[jn0*3+0], aXYZ[jn0*3+1], aXYZ[jn0*3+2]);
    dfm2::CVec3 q1(aXYZ[jn1*3+0], aXYZ[jn1*3+1], aXYZ[jn1*3+2]);
    dfm2::CVec3 q2(aXYZ[jn2*3+0], aXYZ[jn2*3+1], aXYZ[jn2*3+2]);
...
dfm2::CVec3 evaluateField_PotentialFlow
 ...
 const std::vector<double>& aXYZ)
{
  ...
  for (int jtri = 0; jtri<ntri; jtri++){
    dfm2::CVec3 pmj = MidPoint(jtri, aTri, aXYZ);
    dfm2::CVec3 nj = NormalTri(jtri, aTri, aXYZ);
