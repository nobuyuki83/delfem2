...
delfem2::CVec3d delfem2::evaluateField_PotentialFlow_Order1st(
  ...
  for (unsigned int jtri = 0; jtri < aTri.size() / 3; ++jtri) {
    ...
    for (int iint = 0; iint < nint; iint++) {
      ...
      CVec3d dGdndx =
          (1 / (4 * M_PI * len * len * len)) * n - (3 * (v.dot(n)) / (4 * M_PI * len * len * len * len * len)) * v;
...
//    const double elen = sqrt(area*2)*0.5;
    ...
    for (int iint = 0; iint < nint; iint++) {
      ...
      CVec3d dGdnydx =
          (1 / (4 * M_PI * len * len * len)) * ny - (3 * (r.dot(ny)) / (4 * M_PI * len * len * len * len * len)) * r;
      ///
...
delfem2::CMat3d delfem2::gradveloVortexParticle(
  ...
  CMat3d m0 = Mat3_OuterProduct(circ_vp.cross(v), dg0);
  return m0 + g0 * CMat3d(Mat3_Spin(circ_vp));
}
...
void delfem2::viscousityVortexParticleGrid
  ...
  for (unsigned int ivp = 0; ivp < aVortexParticle.size(); ++ivp) {
    ...
    unsigned int igp0 = (ix + 0) * (ny + 1) * (nz + 1) + (iy + 0) * (nz + 1) + (iz + 0);
    double w0 = (1.0 - rx) * (1.0 - ry) * (1.0 - rz);
    unsigned int igp1 = (ix + 1) * (ny + 1) * (nz + 1) + (iy + 0) * (nz + 1) + (iz + 0);
    double w1 = (rx + 0.0) * (1.0 - ry) * (1.0 - rz);
    unsigned int igp2 = (ix + 1) * (ny + 1) * (nz + 1) + (iy + 1) * (nz + 1) + (iz + 0);
    double w2 = (rx + 0.0) * (ry + 0.0) * (1.0 - rz);
    unsigned int igp3 = (ix + 0) * (ny + 1) * (nz + 1) + (iy + 1) * (nz + 1) + (iz + 0);
    double w3 = (1.0 - rx) * (ry + 0.0) * (1.0 - rz);
    unsigned int igp4 = (ix + 0) * (ny + 1) * (nz + 1) + (iy + 0) * (nz + 1) + (iz + 1);
    double w4 = (1.0 - rx) * (1.0 - ry) * (rz + 0.0);
    unsigned int igp5 = (ix + 1) * (ny + 1) * (nz + 1) + (iy + 0) * (nz + 1) + (iz + 1);
    double w5 = (rx + 0.0) * (1.0 - ry) * (rz + 0.0);
    unsigned int igp6 = (ix + 1) * (ny + 1) * (nz + 1) + (iy + 1) * (nz + 1) + (iz + 1);
    double w6 = (rx + 0.0) * (ry + 0.0) * (rz + 0.0);
    unsigned int igp7 = (ix + 0) * (ny + 1) * (nz + 1) + (iy + 1) * (nz + 1) + (iz + 1);
    double w7 = (1.0 - rx) * (ry + 0.0) * (rz + 0.0);
...
std::complex<double> delfem2::evaluateField_Helmholtz_Order1st(
  ...
  for (int jtri = 0; jtri < ntri; jtri++) {
    ...
    COMPLEX aC[3];
    Helmholtz_TransferOrder1st_PntTri(aC, p, q0, q1, q2, k, beta, ngauss);
