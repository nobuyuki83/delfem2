 ...
 const std::vector<dfm2::CContactElement>& aContactElem)
{
  for(int ice=0;ice<aContactElem.size();ice++){
    ...
    dfm2::CVec3 p0( aXYZ[ ino0*3+0], aXYZ[ ino0*3+1], aXYZ[ ino0*3+2] );
    dfm2::CVec3 p1( aXYZ[ ino1*3+0], aXYZ[ ino1*3+1], aXYZ[ ino1*3+2] );
    dfm2::CVec3 p2( aXYZ[ ino2*3+0], aXYZ[ ino2*3+1], aXYZ[ ino2*3+2] );
    dfm2::CVec3 p3( aXYZ[ ino3*3+0], aXYZ[ ino3*3+1], aXYZ[ ino3*3+2] );
    dfm2::CVec3 v0( aUVWm[ino0*3+0], aUVWm[ino0*3+1], aUVWm[ino0*3+2] );
    dfm2::CVec3 v1( aUVWm[ino1*3+0], aUVWm[ino1*3+1], aUVWm[ino1*3+2] );
    dfm2::CVec3 v2( aUVWm[ino2*3+0], aUVWm[ino2*3+1], aUVWm[ino2*3+2] );
    dfm2::CVec3 v3( aUVWm[ino3*3+0], aUVWm[ino3*3+1], aUVWm[ino3*3+2] );
    if( ce.is_fv ){ // face-vtx      
      ...
      double w2 = 1.0 - w0 - w1;
      dfm2::CVec3 pc = w0*p0 + w1*p1 + w2*p2;
      dfm2::CVec3 norm = p3-pc; norm.SetNormalizedVector();
      ...
      dfm2::CVec3 c01 = (1-w01)*p0 + w01*p1;
      dfm2::CVec3 c23 = (1-w23)*p2 + w23*p3;
      dfm2::CVec3 norm = (c23-c01); norm.SetNormalizedVector();
 ...
 const std::vector<dfm2::CContactElement>& aContactElem)
{
  for(int ice=0;ice<aContactElem.size();ice++){
    ...
    if( ce.is_fv ){ // face-vtx
      double w0,w1;
      {        
        dfm2::CVec3 p0m = p0 + t*v0;
        dfm2::CVec3 p1m = p1 + t*v1;
        dfm2::CVec3 p2m = p2 + t*v2;
        dfm2::CVec3 p3m = p3 + t*v3;
 ...
 const std::vector<double>& aUVWm0) // (in) RIZを使う前の中間速度
{
  for(const auto & iriz : aRIZ){
    ...
    dfm2::CVec3 gc(0,0,0); // 重心位置
    dfm2::CVec3 av(0,0,0); // 平均速度
    for(int ino : aInd){
      gc += dfm2::CVec3(aXYZ[  ino*3+0],aXYZ[  ino*3+1],aXYZ[  ino*3+2]);
      av += dfm2::CVec3(aUVWm0[ino*3+0],aUVWm0[ino*3+1],aUVWm0[ino*3+2]);
    }
    gc /= (double)aInd.size();
    av /= (double)aInd.size();
    dfm2::CVec3 L(0,0,0); // 角運動量
    double I[9] = {0,0,0, 0,0,0, 0,0,0}; // 慣性テンソル
    for(int ino : aInd){
      dfm2::CVec3 p(aXYZ[  ino*3+0],aXYZ[  ino*3+1],aXYZ[  ino*3+2]);
      dfm2::CVec3 v(aUVWm0[ino*3+0],aUVWm0[ino*3+1],aUVWm0[ino*3+2]);
      L += Cross(p-gc,v-av);
      dfm2::CVec3 q = p-gc;
