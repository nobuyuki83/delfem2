...
class CKey {    // class for storing values of key
 public:
...
class CMathExpressionEvaluator {
 ...
 private:
  bool m_is_valid;
  std::string m_sExp;
  std::vector<parse_math_expression::CCmd *> m_apCmd;    // 逆ポーランド記法された演算子や値の列
  std::vector<parse_math_expression::CKey> m_aKey;    // 文字列の名前と値が，どのIndexのコマンドに格納されているか
};
