...
DFM2_INLINE void delfem2::makeSplineLoop(
  ...
  for (int icv = 0; icv < nCV; icv++) {
    int icv0 = icv;
    if (icv0 >= nCV) { icv0 -= nCV; }
    int icv1 = icv + 1;
    if (icv1 >= nCV) { icv1 -= nCV; }
    int icv2 = icv + 2;
    if (icv2 >= nCV) { icv2 -= nCV; }
  ...
  std::vector<std::vector<int> > aPoInEd(loopIP0.size());
  {
    for (unsigned int iloop = 0; iloop < nloop; ++iloop) {
      ...
      for (unsigned int ip = 0; ip < np; ip++) {
        ...
        assert(iipo0 >= 0 && iipo0 < (int) loopIP0.size());
        assert(iipo1 >= 0 && iipo1 < (int) loopIP0.size());
        const int ipo0 = loopIP0[iipo0];
        const int ipo1 = loopIP0[iipo1];
        assert(ipo0 >= 0 && ipo0 < (int) aVec2.size());
        assert(ipo1 >= 0 && ipo1 < (int) aVec2.size());
