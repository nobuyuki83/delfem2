...
void dfm2::Print(const tinygltf::Model &model) {
  ...
  for (size_t iac = 0; iac < model.accessors.size(); ++iac) {
    ...
    std::cout << "accessor_idx" << iac << " byteStride: " << ac.ByteStride(model.bufferViews[ac.bufferView])
              << std::endl;
  ...
  for (size_t im = 0; im < model.meshes.size(); ++im) {
    ...
    for (size_t ipri = 0; ipri < mesh.primitives.size(); ++ipri) {
      ...
      for (const auto &attribute: primitive.attributes) {
        std::cout << "mesh" << im << " primitive:" << ipri << " att: " << attribute.first << " acc:" << attribute.second
                  << std::endl;
      }
  ...
  for (size_t in = 0; in < model.nodes.size(); ++in) {
    ...
    if (node.rotation.size() == 4) {
      std::cout << "node_idx: " << in << " rot:" << node.rotation[0] << " " << node.rotation[1] << " "
                << node.rotation[2] << " " << node.rotation[3] << std::endl;
    }
    if (node.translation.size() == 3) {
      std::cout << "node_idx: " << in << " trans:" << node.translation[0] << " " << node.translation[1] << " "
                << node.translation[2] << std::endl;
    }
    assert(node.scale.empty() || node.scale.size() == 3);
    if (node.scale.size() == 3) {
      std::cout << "node_idx: " << in << " scale:" << node.scale[0] << " " << node.scale[1] << " " << node.scale[2]
                << std::endl;
    }
...
void dfm2::GetBoneBinding(
  ...
  for (size_t ij = 0; ij < M.size() / 16; ++ij) {
    ...
    int info;
    CalcInvMat(aBone[ij].affmat3Global, 4, info);
  }
}
...
void dfm2::CGLTF::GetMeshInfo(
  ...
  dfm2::GetMeshInfo(
      vtx_xyz_ini, tri_vtz, skinning_sparse_weight, skinning_sparse_index,
