...
class CBinaryOperator : public CCmd {
 public:
  ...
  bool DoOperation(std::vector<double> &stack) override {
    ...
    switch (m_iOpr) {
      case 0:dleft = dleft + dright;
        return true;
      case 1:dleft = dright - dleft;
        return true;
      case 2:dleft = dleft * dright;
        return true;
      case 3:
        if (fabs(dleft) < 1.0e-20) {
          dleft = 0.0;
          return false;
        }
        dleft = dright / dleft;
        return true;
      case 4:dleft = pow(dright, dleft);
        return true;
      default:std::cout << "Error!-->Illegal iOpr " << m_iOpr << std::endl;
        assert(0);
    }
    return false;
  }
 ...
 public:
  ...
  bool DoOperation(std::vector<double> &stack) override {
    ...
    switch (m_iOpr) {
      ...
      case 1: dleft = -dright;
        return true;
      case 2: dleft = fabs(dright);
        return true;
      case 8: dleft = floor(dright);
        return true;
      case 3: dleft = exp(dright);
        return true;
      case 4: dleft = sin(dright);
        return true;
      case 5: dleft = cos(dright);
        return true;
      case 6:
        if (dright > 1.0e-30) {
          dleft = sqrt(dright);
...
DFM2_INLINE void RemoveExpressionSpaceNewline(std::string &exp) {
...
DFM2_INLINE void RemoveExpressionBracket(std::string &exp) {
...
int GetLowestPriorityOperator(
  ...
  for (unsigned int ipos_curr = 0; ipos_curr < exp.size(); ipos_curr++) {
    ...
    switch (exp[ipos_curr]) {
      case '(':if (iFirstBracket == UINT_MAX) iFirstBracket = ipos_curr;
        iBracketDepth++;
        continue;
      case ')':iBracketDepth--;
        continue;
      default:if (iBracketDepth != 0) continue;
    }
    ...
    switch (exp[ipos_curr]) {
      ...
      case '-':ibegin = ipos_curr;
        ...
        continue;
      case '*':ipriority_curr = 3;
        itype_curr = 3;
        iopr_curr = 2;
        break;
      case '/':ipriority_curr = 4;
        itype_curr = 3;
        iopr_curr = 3;
        break;
      case '^':ipriority_curr = 6;
        itype_curr = 3;
        iopr_curr = 4;
        break;
      default:ipriority_curr = ipriority_max;
  ...
  if (iBracketDepth != 0) {
    std::cerr << "Error!-->Not corresponding bracket" << std::endl;
    return 2;
  }
  if ((ipos_min == 0 || ipos_min == exp.size() - 1) && itype_min == 3) {
    std::cerr << "Error!-->Binary operator misplaced " << std::endl;
    return 9;
  }
  if (iFirstBracket == 0 && ipos_min == UINT_MAX) {
    std::cerr << "Error!-->(hoge)foo" << std::endl;
    return 3;
  }
  // binary operator(+とか-)
...
} // delfem2::parse_math_expression

// above: unexposed
// ---------------------------------------------------------
// below: exposed

delfem2::CMathExpressionEvaluator::~CMathExpressionEvaluator() {
  for (auto &icmd: m_apCmd) {
    delete icmd;
  }
}

void delfem2::CMathExpressionEvaluator::SetKey(
...
bool delfem2::CMathExpressionEvaluator::SetExp(
  const std::string &exp) {
  namespace lcl = delfem2::parse_math_expression;
  m_is_valid = false;
  m_sExp = exp;
  // 消去
  ...
  lcl::RemoveExpressionSpaceNewline(tmp_exp);
  lcl::RemoveExpressionBracket(tmp_exp);
  if (tmp_exp.empty()) {
    m_is_valid = false;
    return false;
  }
  {
    std::vector<lcl::SExpCompo> exp_vec;
...
double delfem2::CMathExpressionEvaluator::Eval() const { // evaluating the math expression
