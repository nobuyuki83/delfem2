...
void delfem2::CCad3D_Face::Initialize(
  ...
  ////
  CVec3d cg, norm;
  cad3d::FaceCenterNormal(cg, norm, aIE, aEdge);
...
void delfem2::MakeItSmooth
  ...
  for (auto &ifc: aFace) {
    ...
    CVec3d nf, cg;
    cad3d::FaceCenterNormal(cg, nf, aIE, aEdge);
    nf.normalize();
...
bool delfem2::MovePointsAlongSketch(
  ...
  //
  CVec3d plane_nrm(0, 0, 0);
  plane_nrm[inorm] = 1;
  CVec3d plane_ex(0, 0, 0);
  plane_ex[(inorm + 1) % 3] = 1;
  CVec3d plane_ey(0, 0, 0);
  plane_ey[(inorm + 2) % 3] = 1;
...
void delfem2::CCad3D::Pick(
  ...
  if (ielem_vtx_picked == 0 || ivtx_picked >= aVertex.size()) {
    ...
    for (unsigned int icp = 0; icp < aVertex.size(); ++icp) {
      ...
      auto pn = Nearest_Line3_Point3<CVec3d>(pos, src_pick, dir_pick);
      if ((pn - pos).norm() < 0.05) {
        ivtx_picked = icp;
        ielem_vtx_picked = 0;
        break;
...
bool delfem2::CCad3D::MouseMotion(
  ...
  if (imode_edit == EDIT_MOVE) {
    ...
    if (iedge_picked >= 0 && iedge_picked < (int) aEdge.size()
        && (ielem_edge_picked == 2 || ielem_edge_picked == 3)) { // moved edge ctrl point
