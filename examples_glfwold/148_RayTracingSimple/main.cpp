    ...
    for(unsigned int iw=0;iw<nwidth;++iw){
      ...
      if( !aIndElem.empty() ) {
        ...
        IntersectionRay_MeshTri3DPart(
            mapDepthPES,
            delfem2::CVec3d(src0), delfem2::CVec3d(dir0),
      ...
      if( pes.itri == UINT_MAX ) {
        ...
        const double dot = n[0]*dir0[0] + n[1]*dir0[1] + n[2]*dir0[2];
...
int main(int argc,char* argv[])
{
  ...
  const unsigned int nwidth = 200;
  const unsigned int nheight = 200;
  std::vector<unsigned char> aRGB(nwidth*nheight*3);
  Hoge(aRGB,
      nheight,nwidth,aNodeBVH,aAABB,aXYZ,aTri);

  dfm2::opengl::CTexRGB_Rect2D tex;
  {
    tex.Initialize(nwidth, nheight, aRGB.data(), "rgb");
    tex.max_x = +1.0;
    tex.min_x = -1.0;
    tex.max_y = +1.0;
    tex.min_y = -1.0;
  ...
  GLFWwindow* window = glfwCreateWindow(nwidth, nheight, "pixels", nullptr, nullptr);
  if ( !window ){
    glfwTerminate();
    return -1;
  }
  ...
  glfwSetWindowPos(viewer.window, 100, 100);
  glfwSetWindowPos(window, 100+nwidth, 100);
