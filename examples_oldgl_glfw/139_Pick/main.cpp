...
int main() {
  ...
  std::vector<unsigned int> tri_flag(tri_vtx.size() / 3, 0);
  ...
  viewer.mousepress_callbacks.emplace_back(
      [&tri_flag, &bvhnodes, &bvhnode_aabb, &tri_vtx, &vtx_xyz](const float src[3], const float dir[3]) {
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    delfem2::opengl::DrawMeshTri3DFlag_FaceNorm(
        vtx_xyz, tri_vtx, tri_flag, aColor);
    // ----------
    viewer.SwapBuffers();
    glfwPollEvents();
  }
