...
int main() {
  ...
  std::vector<unsigned int> tri_vtxidx;  // index of triangles
  ...
  double cur_time = 0;
  {
    ...
    vtx_xyz = vtx_xyz_ini;
    {
      const size_t ntri = tri_vtxidx.size() / 3;
      std::vector<double> aElemCenter(ntri * 3);
      for (unsigned int itri = 0; itri < ntri; ++itri) {
        dfm2::CVec3d p0 = dfm2::CG_Tri3(itri, tri_vtxidx, vtx_xyz);
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    {
      ...
      for (unsigned int ip = 0; ip < vtx_xyz.size() / 3; ip++) {
        vtx_xyz[ip * 3 + 0] = vtx_xyz_ini[ip * 3 + 0] + aUVW[ip * 3 + 0] * d;
        vtx_xyz[ip * 3 + 1] = vtx_xyz_ini[ip * 3 + 1] + aUVW[ip * 3 + 1] * d;
        vtx_xyz[ip * 3 + 2] = vtx_xyz_ini[ip * 3 + 2] + aUVW[ip * 3 + 2] * d;
      }
