...
void DrawVtxEdgeHandler
...

  {
    ...
    for (unsigned int ie = 0; ie < aEdge.size(); ++ie) {
      bool is_loop0 =
          std::find(cad.aIE_picked.begin(), cad.aIE_picked.end(), std::make_pair(ie, true)) != cad.aIE_picked.end();
      bool is_loop1 =
          std::find(cad.aIE_picked.begin(), cad.aIE_picked.end(), std::make_pair(ie, false)) != cad.aIE_picked.end();
