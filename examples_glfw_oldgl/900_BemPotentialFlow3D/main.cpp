...
dfm2::CVector3 posGrid(int ig, int jg, double lenGrid, int ngrid){
  dfm2::CVector3 p;
...
double lenGrid = 1.0;
int ngrid = 45;
std::vector<dfm2::CVector3> aValGrid;
...
void SetProblem()
{
  ...
  for(int ig=0;ig<ngrid+1;ig++){
    for(int jg=0;jg<ngrid+1;jg++){
      dfm2::CVector3 p = posGrid(ig,jg,lenGrid,ngrid);
...
//          double c[4]; dfm2::opengl::heatmap(v, c);
  ...
  
  {
    ...
    for(int jg=0;jg<ngrid+1;jg++){
      dfm2::CVector3 p = posGrid(ig, jg, lenGrid, ngrid);
      dfm2::CVector3 val0 = aValGrid[ig*(ngrid+1)+jg];
      ::glColor3d(1, 0, 0);
