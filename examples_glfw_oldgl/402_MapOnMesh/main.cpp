...
void UpdateProblem() {
  {
    ...
    for( auto o2d : pos2d_org_corner ) {
      dfm2::CVec3d o3d(o2d[0],o2d[1],10);
  ...
  }
  {
    ...
    for(int i=0;i<ndiv+1;++i){
      for(int j=0;j<ndiv+1;++j) {
        ...
        double r2 = ri*rj;
        double r3 = (1-ri)*rj;
        dfm2::CVec3d pA = r0*p0 + r1*p1 + r2*p2 + r3*p3;
        dfm2::CVec3d nA = r0*n0 + r1*n1 + r2*n2 + r3*n3;
        const std::vector<dfm2::CPointElemSurf<double>> aPES = IntersectionLine_MeshTri3D(
            pA, nA,
            aTri, aXYZ);
