...
std::vector<dfm2::CPointElemSurf> aPES0;

const unsigned int ndiv = 8;
std::vector<dfm2::CPointElemSurf> aPES1;
...
void UpdateProblem() {
  {
    aPES0.clear();
    dfm2::CVector3 dir(0,0,-1);
    for( auto o2d : pos2d_org_corner ) {
      dfm2::CVector3 o3d(o2d[0],o2d[1],10);
      std::map<double,dfm2::CPointElemSurf> mapDepthPES;
  ...
  {
    aPES1.clear();
    dfm2::CVector3 p0 = aPES0[0].Pos_Tri(aXYZ,aTri);
    dfm2::CVector3 p1 = aPES0[1].Pos_Tri(aXYZ,aTri);
    dfm2::CVector3 p2 = aPES0[2].Pos_Tri(aXYZ,aTri);
    dfm2::CVector3 p3 = aPES0[3].Pos_Tri(aXYZ,aTri);
    dfm2::CVector3 n0 = aPES0[0].UNorm_Tri(aXYZ,aTri,aNorm);
    dfm2::CVector3 n1 = aPES0[1].UNorm_Tri(aXYZ,aTri,aNorm);
    dfm2::CVector3 n2 = aPES0[2].UNorm_Tri(aXYZ,aTri,aNorm);
    dfm2::CVector3 n3 = aPES0[3].UNorm_Tri(aXYZ,aTri,aNorm);
    for(int i=0;i<ndiv+1;++i){
      for(int j=0;j<ndiv+1;++j) {
        ...
        double r2 = ri*rj;
        double r3 = (1-ri)*rj;
        dfm2::CVector3 pA = r0*p0 + r1*p1 + r2*p2 + r3*p3;
        dfm2::CVector3 nA = r0*n0 + r1*n1 + r2*n2 + r3*n3;
        const std::vector<dfm2::CPointElemSurf> aPES = IntersectionLine_MeshTri3D(
            pA, nA,
            aTri, aXYZ);
        std::map<double,dfm2::CPointElemSurf> mapPES;
        for(auto pes : aPES){
          dfm2::CVector3 p_intersec = pes.Pos_Tri(aXYZ,aTri);
...
void myGlutDisplay()
{
  ...
  for(unsigned int idiv=0;idiv<ndiv;++idiv){
    for(unsigned int jdiv=0;jdiv<ndiv;++jdiv){
      dfm2::CVector3 p0 = aPES1[(idiv+0)*(ndiv+1)+(jdiv+0)].Pos_Tri(aXYZ,aTri);
      dfm2::CVector3 p1 = aPES1[(idiv+1)*(ndiv+1)+(jdiv+0)].Pos_Tri(aXYZ,aTri);
      dfm2::CVector3 p2 = aPES1[(idiv+1)*(ndiv+1)+(jdiv+1)].Pos_Tri(aXYZ,aTri);
      dfm2::CVector3 p3 = aPES1[(idiv+0)*(ndiv+1)+(jdiv+1)].Pos_Tri(aXYZ,aTri);
      if( (idiv+jdiv)%2 == 0 ){
        ::glColor3d(0,0,0);
      }
      else{
