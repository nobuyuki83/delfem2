  ...
  std::vector<unsigned int>& aTri)
{
  ...
  if( !glIsBuffer(vao.VBO_pos) ){
    glGenBuffers(1, &vao.VBO_pos);
  }
  glBindBuffer(GL_ARRAY_BUFFER, vao.VBO_pos); // gl24
  glBufferData(GL_ARRAY_BUFFER, sizeof(float)*(aXYZf.size()), aXYZf.data(), GL_STATIC_DRAW); // gl24
  glEnableVertexAttribArray(0);
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3*sizeof(float), (void*)0); // gl24
  
  if( !glIsBuffer(vao.VBO_nrm) ){
    glGenBuffers(1, &vao.VBO_nrm);
  }
  glBindBuffer(GL_ARRAY_BUFFER, vao.VBO_nrm); // gl24
  glBufferData(GL_ARRAY_BUFFER, sizeof(float)*(aNrmf.size()), aNrmf.data(), GL_STATIC_DRAW); // gl24
