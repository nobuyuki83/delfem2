...
#if defined(USE_GL)
void CSignedDistanceField3D_Plane::Draw() const 
{
...
CSignedDistanceField3D_Sphere::CSignedDistanceField3D_Sphere(double r, double c[3], bool is_out){
	cent_[0] = c[0];
	cent_[1] = c[1];
	cent_[2] = c[2];
	radius_ = r;
...
#if defined(USE_GL)
void CSignedDistanceField3D_Sphere::Draw() const
{
  const bool is_lighting = ::glIsEnabled(GL_LIGHTING);
...
#if defined(USE_GL)
void CSignedDistanceField3D_Cylinder::DrawFace() const
{
//  const bool is_lighting = ::glIsEnabled(GL_LIGHTING);
  const bool is_texture = ::glIsEnabled(GL_TEXTURE_2D);  
  ::glDisable(GL_TEXTURE_2D);  
...
void CSignedDistanceField3D_Cylinder::Draw() const
{
  const bool is_lighting = ::glIsEnabled(GL_LIGHTING);
  const bool is_texture = ::glIsEnabled(GL_TEXTURE_2D);  
  ::glDisable(GL_LIGHTING);
...
#if defined(USE_GL)
void CSignedDistanceField3D_Torus::Draw() const
{
  const bool is_lighting = ::glIsEnabled(GL_LIGHTING);
  const bool is_texture = ::glIsEnabled(GL_TEXTURE_2D);  
  ::glDisable(GL_LIGHTING);
...
void CSignedDistanceField3D_Combine::Draw() const
{
  for(unsigned int ipct=0;ipct<apCT.size();ipct++){ apCT[ipct]->Draw(); } 
}
...
#if defined(USE_GL)
void CSignedDistanceField3D_Transform::Draw() const
{ 
	::glPushMatrix();
	::glTranslated(trans[0],trans[1],trans[2]);
	double rd = 180.0/3.1416;	
	::glRotated(psi*rd,   0,0,1);
	::glRotated(theta*rd, 0,1,0);
	::glRotated(phi*rd,   1,0,0);
	this->pCT->Draw(); 
	::glPopMatrix();
}
#else
...
#if defined(USE_GL)
void CSignedDistanceField3D_Mesh::Draw() const
{  
  ::glColor3d(1,0,0);
  ::glBegin(GL_LINES);
