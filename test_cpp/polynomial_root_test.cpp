  ...
  for (unsigned int itr = 0; itr < 1000; ++itr) {
    ...
    }
    {
      ...
      for (auto intvl: intvls) {
        ...
        EXPECT_EQ(nr, 1);
        auto coeff = std::array<double, 4>{d, c, b, a};
...
TEST(polynomial_root, quartic_4roots) {
  ...
  for (unsigned int itr = 0; itr < 1000; ++itr) {
    ...
    double strum[5][5];
    delfem2::StrumSequenceOfPolynomial<5>(strum, std::array<double,5>{e,d,c,b,a}.data() );
...
TEST(polynomial_root, quintic_5roots) {
  ...
  for (unsigned int itr = 0; itr < 1000; ++itr) {
    ...
    double strum[6][6];
    delfem2::StrumSequenceOfPolynomial<6>(strum, std::array<double,6>{f,e,d,c,b,a}.data() );
